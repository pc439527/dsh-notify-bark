 ...
 */
import { sanitizeBarkUrl } from "./settings-store.js";
 ...
 */
export async function sendBark(endpoint, payload, timeoutMs = 5000) {
    const base = sanitizeBarkUrl(endpoint);
    if (base.length === 0)
        throw new BarkSendError('未配置 Bark 推送地址', BARK_ERROR_CODES.NOT_CONFIGURED);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new BarkSendError(`Bark HTTP ${response.status}${text.length > 0 ? `: ${text}` : ''}`, BARK_ERROR_CODES.HTTP_ERROR);
        }
    }
    catch (error) {
        if (error instanceof BarkSendError)
            throw error;
        const timedOut = error instanceof Error && error.name === 'AbortError';
        if (timedOut)
            throw new BarkSendError(`Bark 请求超时（${timeoutMs}ms）`, BARK_ERROR_CODES.TIMEOUT);
 ...
 */
export function buildTestPayload(group) {
    return {
